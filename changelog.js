...
window.CHANGELOG = [
  {
    v:'4.9.7', d:'24 abr 2026', current:true,
    items:[
      {type:'fix', title:'Manutenção · CRÍTICO · cards de preventiva sumindo do Kanban (race no boot).',
        desc:'BUG CATASTRÓFICO encontrado investigando reclamação do William: "n deveria ficar listado o que o ivonei finalizou? agora tipo sumiu". Os cards concluídos (e qualquer card de preventiva) DESAPARECIAM do Kanban entre sessões.\n\n=== CAUSA (existia desde sempre, ficou exposta agora) ===\n\nNo boot do app, os listeners do Firebase disparam em ordem indeterminada:\n• `onValue(kanbanRef)` recebe os cards reais → state.cards = [N items]\n• `onValue(prevRef)` ainda não disparou → state.preventivas = [] (DEFAULT_PREV)\n\nDentro do callback de kanbanRef, chama `limparCardsOrfaos()`:\n```\nconst prevKeys = new Set(state.preventivas.map(...));  // Set vazio!\nstate.cards.forEach(c => {\n  if (c.tipo === "preventiva" && c.prevId && !prevKeys.has(c.prevId)) {\n    window._fbRemoveCard(c._key);  // ← REMOVE TUDO DO FIREBASE\n  }\n});\n```\n\nResultado: TODOS os cards de preventiva (todo/doing/done) eram removidos do Firebase a cada hard refresh em que kanbanRef carregasse antes de prevRef.\n\nO mesmo padrão estava na minha v4.9.5 (limpeza de fantasmas dentro de gerarCardsPreventivas), também sem guard.\n\n=== FIX v4.9.7 ===\n\n1. `limparCardsOrfaos`: GUARD no início.\n   `if (!state.preventivas || state.preventivas.length === 0) return;`\n   Skip silencioso (com console.warn) se preventivas ainda não carregou.\n\n2. Bloco de limpeza de fantasmas em `gerarCardsPreventivas` (v4.9.5): mesmo guard.\n   Só roda se `state.preventivas.length > 0`.\n\nQuando o onValue(prevRef) finalmente disparar e popular state.preventivas, as funções rodam de novo (chamadas dentro de cada callback) e fazem o trabalho correto.\n\n=== IMPACTO ===\n\n• Cards já apagados do Firebase ESTÃO PERDIDOS — não tem como recuperar (não tem soft-delete no schema atual).\n• Sugestão: rodar restore manual do último backup automático (.github/workflows/backup-firebase.yml roda 2x/dia). William pode baixar o JSON do último backup e mergear via Firebase Console.\n• Pra prevenir: o auto-backup salvou o estado de até 12h atrás.\n\nSub-app Manutenção: v3.40.7 (HUB v4.9.7).'}
    ]
  },
  {
    v:'4.9.6', d:'24 abr 2026',
    items:[
      {type:'fix', title:'Manutenção · diárias não ficam mais como "Atrasada" eternamente (status novo "Rotina diária").',
        desc:'BUG (William): no Plano Preventivo, todas as preventivas DIÁRIAS (freq=1) ficavam com chip vermelho "ATRASADA Xd". Reclamação: "a diaria nao pode ficar em atraso... ele nunca vai dar feita pq nao vai pro kanban".\n\nCAUSA: regra v3.32.1 — preventivas freq=1 NUNCA viram cards no kanban (vão como notificação via Worker Cloudflare). Como o app não tem fluxo pra "executar" diária pelo kanban, `pv.ultima` nunca atualiza → `proxima = ultima_velha + 1d` sempre é passado → status `late` permanente.\n\n=== FIX v4.9.6 ===\n\n1. `_pvStatusFor(p)`: retorna status novo `key:"rotina"` quando `freq=1`:\n   { key:"rotina", lbl:"Rotina diária", dr:null, prox:null }\n\n2. Agregador de contadores da máquina: `rotina` conta em `ok` (não em `late`). Header da máquina deixa de mostrar "⚠ N atrasadas" pra preventivas diárias.\n\n3. Coluna data: pra `rotina`, mostra "Todo dia · executada no turno" (era "21/04 → 22/04 ATRASADA 2D")\n\n4. CSS novo: `.pv-task-status.rotina` com cor roxa neutra (#A5B4FC sobre fundo .13 opaque) — distingue de late (vermelho), due (amarelo), ok (verde).\n\n5. Bonus: `.pv-task-status.sem` (sem data) também ganhou cor explícita (cinza neutro).\n\n=== O QUE NÃO MUDA ===\n• Worker Cloudflare continua enviando notificação push diária ao responsável (manutencao/fcmPending)\n• Dashboard count de "preventivas atrasadas" agora exclui diárias (alinhado com a regra)\n• Calendário e Kanban continuam ignorando freq=1 (regra v3.32.1 intacta)\n\nSub-app Manutenção: v3.40.6 (HUB v4.9.6).'}
    ]
  },
  {
    v:'4.9.5', d:'24 abr 2026',
    items:[
      {type:'fix', title:'Manutenção · card fantasma de Preventiva ao concluir grupo (race condition).',
        desc:'BUG (William): após finalizar uma preventiva semanal (5 tarefas concluídas via checklist), o sistema gerou imediatamente um NOVO card no "A Fazer" — mas como é semanal, só deveria voltar daqui 7 dias. "nao faz sentido aparecer já, falta 7 dias ainda".\n\n=== CAUSA: RACE CONDITION ===\n1. _moverGrupo move N cards pra "done" + atualiza `pv.ultima = hoje` no Firebase\n2. onValue de `manutencao/kanban` dispara PRIMEIRO (pq o update de cards é o mais frequente)\n3. renderKanban → gerarCardsPreventivas roda\n4. Mas state.preventivas AINDA tem ultima ANTIGA (onValue de preventivas não disparou ainda)\n5. proxima = ultima_antiga + freq = HOJE → dr=0 → vencida → CRIA CARD\n6. Logo depois, onValue de preventivas dispara → state.preventivas atualizado → mas `jaExiste = true` impede limpeza\n7. Card fantasma fica visível em "A Fazer"\n\n=== FIX v4.9.5 ===\nNa `gerarCardsPreventivas`, ANTES de criar novos cards, varre state.cards e remove cards "todo" tipo=preventiva cuja preventiva associada NÃO está mais vencida (proxima > hoje).\n\nLógica:\n• Se card é preventiva + col=todo + tem prevId\n• Acha pv correspondente (state.preventivas)\n• Se pv tem freq + ultima → calcula proxima\n• Se proxima > hoje → não venceu mais → fantasma → `_fbRemoveCard(c._key)`\n\nRegras preservadas:\n• Cards "doing" e "done" NUNCA são tocados\n• Preventivas SEM `ultima` (vencidas-desde-sempre) mantêm o card\n• Preventivas diárias (freq=1) continuam fora do auto-card (regra v3.32.1)\n• Hierarquia maxFreq por máquina intacta (Opção B v3.50.0)\n\nDiferença vs limparCardsFantasma (v3.48.6): aquela era one-shot via flag localStorage só pra admin. Esta roda toda chamada de gerarCardsPreventivas pra prevenir o problema continuamente.\n\nSub-app Manutenção: v3.40.5 (HUB v4.9.5).'}
    ]
  },
  {
    v:'4.9.4', d:'24 abr 2026',
    items:[
      {type:'fix', title:'Manutenção · checklist de Preventiva agrupada agora PERSISTE no Firebase.',
        desc:'BUG (reportado pelo William): no modal "Preventiva Semanal · 5 tarefas" (e similares), os checks marcados desapareciam ao fechar e reabrir o modal. Reclamação: "deveria ficar o checklist feito, mas nao salva".\n\nCAUSA: `_toggleChecklistItem` em manutencao/index.html só fazia `card.checklistDone = !card.checklistDone` — alteração APENAS no array `_grupoAtual.cards` em memória. Nenhum push pra Firebase. Quando o modal era reaberto, `_renderChecklistGrupo` recria a lista a partir de state.kanban (que continha o estado original) → todos os checks zeravam.\n\nFIX:\n• `_toggleChecklistItem`: depois do toggle local, chama `window._fbUpdateCard(cardKey, { checklistDone: card.checklistDone })` pra persistir.\n• Espelha no `state.kanban[cardKey].checklistDone` pra UI permanecer consistente sem esperar onValue do Firebase.\n• `_concluirTodasGrupo`: aplica o mesmo (loop de cards marca true + Firebase + state).\n\nFluxo agora: marca check → grava em `manutencao/kanban/{cardKey}/checklistDone:true` no RTDB → fecha modal → reabre → check ainda lá. Se completar todos → AUTO-MOVE pra "done" como antes (zero mudança de fluxo).\n\nSchema novo no card: `checklistDone:bool` (default undefined/false). Campo opcional, retrocompat 100%.\n\nSub-app Manutenção: v3.40.4 (HUB v4.9.4 · cache-buster automático no iframe).'}
    ]
  },
  {
    v:'4.9.3', d:'24 abr 2026',
    items:[
  ...
  {
    v:'4.9.0', d:'24 abr 2026',
    items:[
      {type:'feat', title:'Apontamento Tintoria · redesign completo (desktop 2 colunas · hero + turnos tabs · fibras com cor).',
        desc:'Mockup mockup-apontamento-redesign aprovado pelo William.\n\n=== REDESIGN COMPLETO ===\n\nDESKTOP · grid 340px / 1fr:\n• LEFT rail: calendário compacto + legenda + KPIs do mês (Total + Média/dia)\n• RIGHT day-panel dominante: hero + tabs turnos + fibras + footer\n\nHero do dia:\n• Número grande Outfit Black 900 (3.4rem) com kerning negativo\n• Meta: mês/ano (DM Mono verde) + dia semana completo + "Hoje · Semana N"\n• Total do dia na direita (Outfit 800 2rem verde) · auto-atualiza conforme digita\n\nTurnos como tabs segmentadas:\n• Bullet muted → verde quando filled\n• Time range (06h-14h, 14h-22h, 22h-06h) em DM Mono muted\n• Responsável (auto-detectado via td.by ou USERS.turno) + " · você" quando match\n• Total kg do turno (DM Mono 600) · "—" se vazio\n• Cadeado pra turnos sem permissão\n\n4 fibras como cards COLORIDOS POR TIPO (paleta v4.1):\n• CO = azul #0A84FF\n• PAC = amarelo #FFCB00 (light mode #A07A00)\n• CV = verde #3ECF6E\n• PES = roxo #BF5AF2\n• Border-top 3px colorido + chip com mesma cor (bg 15% opaque)\n• Inputs Outfit Black 800 1.55rem · HUGE como display de balança industrial\n\nFooter:\n• Esquerda: "Turno · subtotal" + "Horas trabalhadas"\n• Direita: botão "Salvar turno 1º/2º/3º" (dinâmico conforme turno ativo)\n\n=== HELPER NOVO ===\naptFmtKgShort(v): 142800 → "142,8 t" / 8420 → "8,4 t" / 420 → "420 kg"\nUsado em: hero total, KPIs do mês, total kg por turno na tab.\n\n=== MOBILE (≤768px) ===\n• Shell colapsa pra 1 coluna (rail acima do day-panel)\n• Hero com número menor (3.4→2.4rem) + total 2→1.4rem\n• Tabs viram scroll horizontal · cada tab min-width 108px\n• Fibras grid 2x2 (era 4 inline) · input 1.55→1.3rem\n• Footer empilhado com botão full-width\n• Card do mês padding menor\n\n=== COMPAT / LÓGICA INTACTA ===\nMantidos os IDs usados pelo JS antigo:\n• apt-cal-grid, apt-cal-month, apt-badge-dias\n• apt-day-date (hidden), apt-turno-tabs, apt-turno-panel\n• apt-readonly-banner, apt-readonly-sub\n• apt-foot-horas, apt-foot-kg, apt-btn-save, apt-btn-save-lbl\n• apt-in-horas, apt-in-co/pac/pes/cv, apt-total-turno\n\nFunções preservadas 100%:\n• aptMesNav, aptRenderCalendario, aptRenderDia, aptCapturarRascunho\n• aptAtualizarTotais, salvarApontamento, aptPodeEditar\n• APT_TURNOS, APT_FIBRAS, aptFmtKg/Mes/Dia, aptParseNum\n\n=== ARQUIVOS TOCADOS ===\n• index.html: HTML panel-apontamento reescrito + aptRenderCalendario/Dia estendidos + aptFmtKgShort novo\n• css/hub.css: ~250 linhas novas escopadas em #panel-apontamento + esconde .apt-cal/.apt-day-title/.apt-foot/.apt-tp-card legacy\n\n=== FIM do ciclo v4.7/4.8/4.9 ===\n• v4.7.0 Cor compactação + paginação\n• v4.8.0 Timeline desktop dual-col\n• v4.9.0 Apontamento redesign (este)'}
    ]
  },
  {
    v:'4.8.0', d:'24 abr 2026',
    items:[
