...
>
> **Versão do doc:** 2.46 — 22/04/2026
> **Versão atual do HUB:** v3.40.0
...
| `css/tokens.css` | **Design tokens compartilhados** (v3.27.0). 30+ vars semânticas. Linkado por todos os 4 apps antes do CSS específico. Single source of truth. |
| `css/hub.css` | CSS principal extraído (v3.0.0). Linkado via `<link rel="stylesheet" href="css/hub.css?v=X.X.X">` (cache-buster). |
| `changelog.js` | Array de versões (lazy, v3.25.1). Carregado só ao abrir Central de Atualizações. |
...
| **Manutenção** | | |
| `manutencao/kanban`, `manutencao/pecas`, `manutencao/preventivas`, `manutencao/maquinas`, `manutencao/historico`, `manutencao/fcmTokens`, `manutencao/fcmPending` | Dados do Manutenção. Máquinas (v3.34.0): `{nome, tag, setor, modelo, ano, prevs, status?, responsavel?, localizacao?}`. | varia |
...
| Versão | Marco |
|---|---|
| v3.40.0 | HUB · 3 ajustes estruturais: (1) **crop interativo** de imagem ao adicionar avatar (modal com zoom slider + drag, 400×400 JPEG 0.85, usado em Minha Conta + Editar Usuário); (2) **poderes condicionais** ao acesso ao módulo Manutenção (toggles disabled + aviso vermelho quando user não tem Manutenção marcado); (3) **migração USERS hardcoded → users-config**: seed 1x copia os 17 users do código pra Firebase, `isUserDinamico` agora retorna true pra todos (exceto `admin` protegido), admin edita/exclui qualquer user via UI. Flag `fiobras-users-seed-v1`. |
...
| v3.32.2 | Manutenção · switch de notificações no header 🔔/🔕 (Item 6). Popup automático removido. Flag `fio_notif_silenciar` respeitada em `mostrarNotif`. |
| v3.32.1 | Manutenção · frequência Diária (1d) no form de preventiva. Filtrada do calendário e do Kanban (só notificação, Item 7 do ciclo v3.10). |
| v3.32.0 | Manutenção · cards de Máquinas compactos (Item 5 do ciclo v3.10). Código (m.tag) como chip verde, lista de preventivas sai do card (só no modal de edição). Altura 500→110px. |
| v3.31.3 | **FIX REAL** Manutenção: `CHECK_SVG_STR`/`EDIT_SVG_STR`/`DEL_SVG_STR` nunca tinham sido definidas — ReferenceError silencioso apagava o tbody. + Resumo "enquanto você estava fora" só mostra 1x (last-seen por user em localStorage). |
...
| v3.29.0 | Imagens com `loading="lazy"` + `decoding="async"` (12 `<img>` em HUB e Manutenção). Timeline com 4+ fotos economiza ~200KB. |
...
### 9.0.1 Border-radius (v3.26.0)
Escala universal de 6 valores. **Todo novo `border-radius` deve usar a escala:**
- `4px` (`--t-radius-xs`) — badges, micro-tags, progress bars
- `8px` (`--t-radius-sm`) — inputs, buttons, chips
- `12px` (`--t-radius-md`) — cards, painéis pequenos
- `16px` (`--t-radius-lg`) — modais, painéis grandes
- `20px` (`--t-radius-xl`) — pills arredondadas grandes
- `999px` (`--t-radius-pill`) — pílulas totais

CSS vars no `:root` do `hub.css`. Não usar valores intermediários (5, 7, 10, 14 etc).
